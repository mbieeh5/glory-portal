      ...
      <div className="max-w-4xl">
        ...
        <div className={`p-3 rounded-lg border ${isDark ? 'bg-gray-800/50 border-gray-700' : 'bg-white border-gray-200'}`}>
          ...
          <div className="text-xs">
            ...
            <div className="flex items-start">
              <span className="text-gray-500 dark:text-gray-400 w-24 flex-shrink-0">Invoice ID</span>
              <span className="font-medium text-gray-900 dark:text-white">: {row.invoice_id}</span>
            </div>
            <div className="flex items-start">
        ...
        <div className={`p-3 rounded-lg border ${isDark ? 'bg-gray-800/50 border-gray-700' : 'bg-white border-gray-200'}`}>
            <h4 className='text-xs font-bold text-gray-500 dark:text-gray-400 mb-2 uppercase tracking-wider flex items-center gap-2'>
             <UserXIcon size={12} /> Teknisi
            </h4>
            <div className="text-xs">
                <div className="flex items-start">
                    <span className="text-gray-500 dark:text-gray-400 w-24 flex-shrink-0">Teknisi</span>
                    <span className="text-gray-900 dark:text-white flex-1">: {row.technician}</span>
                </div>
                {row.technicial_fee && (
                <div className="flex items-start">
                    <span className="text-gray-500 dark:text-gray-400 w-24 flex-shrink-0">Keluhan</span>
                    <span className="text-gray-900 dark:text-white flex-1">: {row.technicial_fee}</span>
                </div>
                )}
            </div>
        </div>
        ...
        <div className={`p-3 rounded-lg border ${isDark ? 'bg-gray-800/50 border-gray-700' : 'bg-white border-gray-200'}`}>
          ...
          <div className="text-xs">
            <div className="flex items-start">
              <span className="text-gray-500 dark:text-gray-400 w-24 flex-shrink-0">Estimasi Harga</span>
              <span className="font-medium text-gray-900 dark:text-white">: Rp {row.initial_price.toLocaleString('id-ID')}</span>
            </div>
            <div className="flex items-start">
              <span className="text-gray-500 dark:text-gray-400 w-24 flex-shrink-0">Harga Akhir</span>
                ...
                >
                  <div className="text-xs">
                    ...
                    {part.sparepart_variant && (
                      <div className="flex items-start">
                        <span className="text-gray-500 dark:text-gray-400 w-20 flex-shrink-0">Variant</span>
  ...
  const fetchData = useCallback(async (page: number) => {
    ...
    try {
      const fetchCount = async (statusFilter?:string) => {
        let q = supabase.schema('glory')
        .from('services_transactions').select('status',{count:'exact', head: true});
        if(statusFilter) q = q.eq('status', statusFilter)
        const { count } = await q;
        return count || 0
      }
      const [total, completed, inProcess, canceled] = await Promise.all([
        fetchCount(), fetchCount('completed'), fetchCount('in_process'), fetchCount('canceled')
      ]);
    ...
    () => [
      ...
      },
      {
        ...
        cell: ({ getValue }) => {
          ...
          const locations: Record<string, string> = {SKHT: 'Sukahati', CKRT: 'Cikaret'}
          const getLoct = invId.split('GPS-')[1]?.substring(0,4);
          const prefix = locations[getLoct] || "unknown"

          return (
            <span
              className="font-mono font-semibold text-orange-600 dark:text-orange-400 transition-all text-left whitespace-nowrap"
            >
              {prefix}
            </span>
          );
      ...
      <div className="max-w-[1600px] mx-auto">
        ...
        >
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 md:gap-4 mt-4 md:mt-6 pt-4 md:pt-6 border-t border-gray-200 dark:border-gray-700">
            ...
            <div>
              <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Selesai Belum DiAmbil</p>
              <p className="text-xl md:text-2xl font-bold text-green-600 dark:text-green-400">
                {stats.completed_not_pick}
              </p>
            </div>
            <div>
            ...
            <div>
              <p className="text-xs text-gray-500 dark:text-gray-400 mb-1">Batal Belum DiAmbil</p>
              <p className="text-xl md:text-2xl font-bold text-red-600 dark:text-red-600">
                {stats.canceled_not_pick}
              </p>
            </div>
          </div>
        </div>
                    ...
                    <React.Fragment key={row.id}>
                      ...
                      >
                        {row.getVisibleCells().map((cell) => (
                          <td key={cell.id} className="px-2 lg:px-4 py-1 lg:py-2 text-sm text-gray-900 dark:text-white">

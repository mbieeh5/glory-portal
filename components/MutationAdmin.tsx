...
const StatusIcon = ({ status }: { status: StatusBankEnum }) => {
  if (status === StatusBankEnum.COMPLETED) {
    return (
      <div className="w-7 h-7 rounded-full bg-emerald-500 flex items-center justify-center shadow-sm shrink-0">
  ...
  if (status === StatusBankEnum.PENDING) {
    return (
      <div className="w-7 h-7 rounded-full bg-amber-500 flex items-center justify-center shadow-sm shrink-0">
        <Clock className="w-4 h-4 text-white" strokeWidth={2} />
      </div>
    );
  }
  return (
    <div className="w-7 h-7 rounded-full bg-red-400 dark:bg-red-500/80 flex items-center justify-center shadow-sm shrink-0">
...
export default function MutationsAdmin() {
  ...
  const SortButton = ({ label, sortForKey }: { label: string; sortForKey: SortKey }) => (
    <button
      onClick={() => toggleSort(sortForKey)}
      ...
      <div className="flex flex-col lg:flex-row gap-6 max-w-screen-2xl mx-auto">
        ...
        <div className="flex-1 min-w-0">
          ...
          <div className="mb-5 flex flex-col md:flex-row gap-4 justify-between items-start">
            ...
            <div className="flex gap-3 w-full md:w-auto overflow-x-auto pb-1 md:pb-0">
              <div className="bg-white dark:bg-zinc-900 p-3.5 rounded-xl shadow-sm border border-slate-200 dark:border-zinc-800 flex-1 min-w-[150px]">
                ...
                {groupedData.map((group) => (
                  <div key={group.key}>
                    ...
                    {group.items.map((t) => {
                      ...
                      const sorting = SortButton({ label: "Tanggal", sortForKey: "entry_datetime" });

                      return (
                        <div
                          key={t.id}
                          className={`group flex items-start gap-3 px-3 sm:px-6 py-3.5 transition-colors ${
                            isSelected
                        ...
                        >
                          ...
                          <div className="flex flex-col items-center justify-center shrink-0 pt-6">
                            <button
                              ...
                              className="mt-1.5 p-1 -ml-1 shrink-0 rounded hover:bg-slate-200 dark:hover:bg-zinc-800 text-slate-400 dark:text-zinc-500"
                              >
                              {isSelected ? (
                          ...
                          <div className="hidden xs:flex sm:flex flex-col items-center justify-center w-11 sm:w-12 shrink-0 rounded-lg bg-slate-50 dark:bg-zinc-950 border border-slate-200 dark:border-zinc-800 py-1.5">
                          <div className="pb-1">
                          ...
                          <div className="flex-1 min-w-0 flex flex-col gap-0.5 sm:gap-1 overflow-hidden">
                            <div className="flex items-start justify-between gap-2">
                              ...
                              <button
                                ...
                                className="p-1 rounded text-slate-400 dark:text-zinc-500 opacity-0 group-hover:opacity-100 hover:bg-slate-200 dark:hover:bg-zinc-800 hover:text-slate-700 dark:hover:text-zinc-200 transition-all shrink-0"
                              >
        ...
        {selectedRows.length > 0 && (
          <div className="w-full lg:w-80 flex-shrink-0 flex flex-col gap-4">
            <div className="bg-white dark:bg-zinc-900 border border-slate-200 dark:border-zinc-800 p-5 rounded-2xl shadow-sm lg:sticky lg:top-6 h-fit max-h-[90vh] flex flex-col">
